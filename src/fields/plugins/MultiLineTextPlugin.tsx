...
import type { MultiLineTextField } from '../../types/schema';
import { FIELD_TYPES, UI_STRINGS, DEPENDENCY_OPERATORS, CONFIG } from '../../constants';
...
export const MultiLineTextPlugin: FieldPlugin<MultiLineTextField> = {
  type: FIELD_TYPES.MULTI_LINE_TEXT,
  label: UI_STRINGS.FIELD_TYPE_LABELS[FIELD_TYPES.MULTI_LINE_TEXT],
  ...
  FillerComponent: ({ field, value, onChange, error, required, readOnly }) => (
    <div className={rendererStyles.fieldContainer}>
      ...
      <textarea
        className={rendererStyles.textarea}
        value={value || ''}
        onChange={(e: React.ChangeEvent<HTMLTextAreaElement>) => onChange(e.target.value)}
        placeholder={readOnly ? '' : field.placeholder}
        disabled={readOnly}
        rows={field.rows || CONFIG.DEFAULT_ROWS}
      />
